...
class TestDifferentTablesIndependent:
    ...
    def test_different_tables_have_no_shared_resource_ids(self) -> None:
        ...
        assert resources_a.isdisjoint(resources_b), (
            f"Expected no shared resource IDs for different tables, "
            f"but got overlap: {resources_a & resources_b}"
        )
...
class TestSameTableReadReadIndependent:
    ...
    def test_multiple_threads_select_same_table_all_report_read(self) -> None:
        ...
        assert len(stats_events) == 5, (
            f"Expected 5 read events (one per thread), got {len(stats_events)}"
        )
...
class TestReadWriteConflict:
    ...
    def test_read_and_write_same_table_both_report_resource(self) -> None:
        ...
        assert len(reader_reads) >= 1, (
            f"Reader should report read from sql:balances, got {results['reader']}"
        )
        ...
        assert len(writer_writes) >= 1, (
            f"Writer should report write to sql:balances, got {results['writer']}"
        )
...
class TestMultiTableResourceIds:
    ...
    def test_threads_on_partially_overlapping_tables_separate_correctly(self) -> None:
        ...
        def thread_b() -> None:
            ...
            cur = conn_b.cursor()
            cur.execute(
                "SELECT o.amount, s.status FROM orders o JOIN shipments s ON o.id = s.order_id"
            )
            cur.fetchall()
            with lock:
                results["b"].extend(thread_log.events)
        ...
        assert "orders" in (tables_a & tables_b), (
            f"orders should appear in both threads: A={tables_a}, B={tables_b}"
        )
        ...
        assert "shipments" not in tables_a, (
            f"shipments should only be in Thread B, but also in A: {tables_a}"
        )
    ...
    def test_parameterized_writes_report_same_resource(self) -> None:
        ...
        threads = [
            threading.Thread(target=writer, args=(i, f"action_{i}"))
            for i in range(1, 5)
        ]
        for t in threads:
            t.start()
        ...
        assert len(all_write_resources) >= 4, (
            f"Expected at least 4 write reports, got {len(all_write_resources)}"
        )
